...
VESC_MOTOR_INFO VESC_MOTO_INFO[6] = {0};
...
void comm_can_set_duty(uint8_t controller_id, float duty)
{
    ...
    comm_can_transmit_extid(&hcan2, controller_id |
...
void comm_can_set_current_rel(uint8_t controller_id, float current_rel)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index, 0);
    comm_can_transmit_extid(&hcan2, controller_id |
...
void comm_can_set_handbrake(uint8_t controller_id, float current)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    buffer_append_float32(buffer, current, 1e3, &send_index, 0);
    comm_can_transmit_extid(&hcan2, controller_id |
...
void comm_can_conf_current_limits(uint8_t controller_id, bool store, float min, float max)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_float32(buffer, min, 1e3, &send_index, 0);
    buffer_append_float32(buffer, max, 1e3, &send_index, 0);
    comm_can_transmit_extid(&hcan2, controller_id |
