...
void M3508_Send_Currents(void)
{
	...
	{
		CAN2_TxData_2[0] = (uint8_t)(MOTO_REAL_INFO[4].GM6020.set_voltage >> 8);
		CAN2_TxData_2[1] = (uint8_t) MOTO_REAL_INFO[4].GM6020.set_voltage;	      //ID 5 
	}
	else
	{
	...
	if(MOTO_REAL_INFO[4].Motor_Type == M_6020)
	{
		CAN2_TxData_2[2] = (uint8_t)(MOTO_REAL_INFO[5].GM6020.set_voltage >> 8);
		CAN2_TxData_2[3] = (uint8_t) MOTO_REAL_INFO[5].GM6020.set_voltage;	      //ID 6
	}
	else
	{
	...
	if(MOTO_REAL_INFO[4].Motor_Type == M_6020)
	{
		CAN2_TxData_2[4] = (uint8_t)(MOTO_REAL_INFO[6].GM6020.set_voltage >> 8);
		CAN2_TxData_2[5] = (uint8_t) MOTO_REAL_INFO[6].GM6020.set_voltage;	      //ID 7
	}
	else
	{
	...
	if(MOTO_REAL_INFO[4].Motor_Type == M_6020)
	{
		CAN2_TxData_2[6] = (uint8_t)(MOTO_REAL_INFO[7].GM6020.set_voltage >> 8);
		CAN2_TxData_2[7] = (uint8_t) MOTO_REAL_INFO[7].GM6020.set_voltage;	      //ID 8
	}
	else
	{
    ...
    switch(msg -> StdId)  // 检测标准ID
	{
		...
		{ 
			if(MOTO_REAL_INFO[4].Motor_Type == M_6020)
			{
				...
				MOTO_REAL_INFO[4].GM6020.temp =    Data[6];
			}
			else
			{
				// for(int i=0; i<8; i++)
					// CONTROL.CAN1_Send_Msg[4][i] = Data[i];
		...
		{ 
			if(MOTO_REAL_INFO[5].Motor_Type == M_6020)
			{
				...
				MOTO_REAL_INFO[5].GM6020.temp =    Data[6];
			}
			else
			{
				// for(int i=0; i<8; i++)
					// CONTROL.CAN1_Send_Msg[5][i] = Data[i];
		...
		{ 
			if(MOTO_REAL_INFO[6].Motor_Type == M_6020)
			{
				...
				MOTO_REAL_INFO[6].GM6020.temp =    Data[6];
			}
			else
			{
				// for(int i=0; i<8; i++)
					// CONTROL.CAN1_Send_Msg[6][i] = Data[i];
		...
		case M3508_M8_ID://顶部的2006
		{
			if(MOTO_REAL_INFO[7].Motor_Type == M_6020)
			{
				...
				MOTO_REAL_INFO[7].GM6020.temp =    Data[6];
			}
			else
			{ 
				// for(int i=0; i<8; i++)
					// CONTROL.CAN1_Send_Msg[7][i] = Data[i];
